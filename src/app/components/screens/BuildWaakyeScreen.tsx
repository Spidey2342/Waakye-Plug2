...
export function BuildWaakyeScreen({ order, onUpdateOrder, onBack, onContinue }: BuildWaakyeScreenProps) {
  ...
  const proteinCount = Object.values(order.proteins).reduce((a, b) => a + b, 0);
    ...
    <div className="min-h-[100dvh] bg-[#fefaf4] flex flex-col [webkit-tap-highlight-color:transparent]">
      ...
      <div className="flex-1 overflow-y-auto pb-40 [-webkit-overflow-scrolling:touch]">
        <div className="max-w-2xl mx-auto p-4 space-y-6">
          ...
          >
            <div className="w-16 h-16 rounded-2xl bg-[#7a1d1d]/10 flex items-center justify-center text-4xl shrink-0">
              🍚
            </div>
            <div>
              <h2 className="font-bold text-xl">Waakye Bowl</h2>
              <p className="text-sm text-gray-500">🔥 Freshly prepared this morning</p>
            </div>
          </motion.div>
          ...
          >
            <div className="bg-white rounded-xl border border-gray-100 py-3 text-center">
              <p className="font-bold text-sm">{BOWL_SIZES[order.size].name}</p>
              <p className="text-[11px] text-gray-500">Size</p>
            </div>
            <div className="bg-white rounded-xl border border-gray-100 py-3 text-center">
              <p className="font-bold text-sm">{proteinCount || '—'}</p>
              <p className="text-[11px] text-gray-500">Proteins</p>
            </div>
            <div className="bg-white rounded-xl border border-gray-100 py-3 text-center">
              <p className="font-bold text-sm">{extraCount || '—'}</p>
          ...
          >
            <h2 className="font-bold text-base mb-3">Choose Your Size</h2>
          ...
          >
            <h2 className="font-bold text-base mb-3">Add Proteins</h2>
          ...
          >
            <h2 className="font-bold text-base mb-3">Extras</h2>
            <div className="space-y-2">
              {EXTRAS.map((extra) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Checkbox checked={isChecked} onCheckedChange={() => toggleExtra(extra.id)} />
                      <div>
