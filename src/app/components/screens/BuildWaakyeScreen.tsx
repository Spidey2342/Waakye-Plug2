...
export function BuildWaakyeScreen({ onBack, onAddToCart }: BuildWaakyeScreenProps) {
  ...
  const menuIsEmpty = menu.base.length === 0 && menu.protein.length === 0 && menu.extra.length === 0;
                  ...
                  return (
                    <button
                      ...
                      className={`relative p-4 rounded-xl border-2 transition-all ${
                        isSelected ? 'border-[#7a1d1d] bg-[#7a1d1d]/5' : 'border-gray-200 bg-white hover:border-gray-300'
                      }`}
                    >
                      {isSelected && (
